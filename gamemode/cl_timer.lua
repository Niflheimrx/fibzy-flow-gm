...
local function GetTimePiece( nCompare, nStyle )
	...
	local nDifference = nFirst - nCompare
	local nAbs = ab( nDifference ) * 100 / 100

	if nDifference < 0 then
		return fo( "%.1d.%.1d", fl( nAbs / 60 ), fl( nAbs % 60 )  )
...
function GM:HUDPaintBackground()
	...
	if lpc:Team() == ts then
		...
		if IsValid( ob ) and ob:IsPlayer() then
			...
			-- pogress
			if ob:IsBot() then
				draw.SimpleText("Progress "  .. GetTimePiece( nCurrent ) .. "%", "HUDTimer", ScrW() / 2, yPos - 45, text, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
			end
		...
		HUD:Draw(2, lpc, {pos = {Xo, Yo}, pb = Tbest, current = nCurrent, curTp = GetTimePieceOld(nCurrent, nStyle), recTp = GetTimePieceOld(Tbest, nStyle)})
